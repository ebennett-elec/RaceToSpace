...
#pragma mark - Connection and uthorization
/** @name Authorization */
/**
 Invoked when die fails authorization.
...
/**
 Invoked when the die is properly authorized.
 
 @param die The die object initiating the callback.
 */
//- (void)dieAuthorized:(DPDie*)die;

@optional

//TODO debug
//- (void)die:(DPDie*)die didUpdateDebugLog:(NSString*)debug;
...
/**
 ...
 @param die The die object initiating the callback.
 @param charging Indicates if the battery is currently being charged.
 @param batteryPercentage The current battery level in the range from 0 to 100.
 @param error If unsuccessful, set with the encountered failure.
 */
- (void)die:(DPDie*)die didUpdateBatteryStatus:(BOOL)charging andPercentage:(uint8_t)batteryPercentage error:(NSError*)error;
...
/**
 Invoked after setting the notification configuration for sensors.
 
 @param die The die object initiating the callback.
 @param sensor The sensor which config was set.
 @param error "nil" value if successfull, otherwise the error object wrapping encountered
 failure.
 */
//- (void)die:(DPDie*)die didUpdateNotificationConfigurationStateForSensor:(DPSensor)sensor withError:(NSError*)error;

/**
 Invoked when reading value for sensor fails.
 
 @param die The die object initiating the callback.
 @param characteristic The characteristic which value failed to be read.
 @param error The error object wrapping encountered failure.
 */
//- (void)die:(DPDie*)die readFailedForCharacteristic:(DPCharacteristic)characteristic withError:(NSError*)error;
